...
Danna ƙasa domin karɓa:""",
    ...
    # ===== GROUP FORMAT (IYA NAN AKA GYARA) =====
...
def deliver_movies(call):
    ...
    if not movies:
        bot.send_message(user_id, "❌ Babu fim daka taba siya")
        return
...
def show_cart(chat_id, user_id):
    ...
    kb.row(InlineKeyboardButton("⤴️KOMA FARKO", callback_data="go_home"),
           InlineKeyboardButton("🫂Our Channel", url=f"https://t.me/{CHANNEL.lstrip('@')}"))
    # change language
    change_label = tr_user(user_id, "change_language_button", default="🌐 Change your language")
    kb.row(InlineKeyboardButton(change_label, callback_data="change_language"))
...
def handle_callback(c):
    ...
    if data == "my_movies":
        kb = InlineKeyboardMarkup()
        kb.add(InlineKeyboardButton("🗓 This week", callback_data="resend:7"))
        kb.add(InlineKeyboardButton("📆 Months ago", callback_data="resend:120"))
        kb.add(InlineKeyboardButton("🕰 Years ago", callback_data="resend:3650"))

        bot.send_message(
            uid,
            "🎬 <b>My Movies</b>\nZa a sake turo maka duk fina-finan da ka taɓa siya.",
            reply_markup=kb
        )
        bot.answer_callback_query(c.id)
        return
    ...
    if data.startswith("resend:"):
        days = int(data.split(":")[1])  # (ana barinsa, ko da ba a yi amfani da shi ba)
        ...
        rows = conn.execute("""
            ...
            WHERE um.user_id = ?
            ORDER BY um.rowid ASC
        """, (uid,)).fetchall()

        if not rows:
            bot.send_message(uid, "❌ Babu fim da ka taba siya.")
            bot.answer_callback_query(c.id)
            return
   ...
   # ================= CALLBACK HANDLER =================
    ...
    if data == "checkout":
        ...
        # ==================================================
        # 1️⃣ KARIYA: DUBA KO AKWAI UNPAID ORDER TUN TUNI
        # ==================================================
        old = conn.execute(
            """
            SELECT id, amount
        ...
        if old:
            order_id = old["id"]
            remaining_price = int(old["amount"] or 0)
            ...
            if not pay_url:
                bot.answer_callback_query(c.id, "❌ Kuskure wajen dawo da payment link.")
                return
            ...
            bot.send_message(
                uid,
                f"""⚠️ <b>Kana da CART order da bai kammala ba</b>

🧾 <b>Order ID:</b>
<code>{order_id}</code>

💵 <b>Jimilla:</b> ₦{remaining_price}
...
👇 Ci gaba da biyan wannan order:""",
    ...
    if data.startswith("groupitem:"):
        ...
        # 🔢 ADADIN MOVIES A SERIES (GYARA ANAN)
        movie_count = conn.execute(
        ...
        # 🔒 1️⃣ DUBA KO AKWAI TSOHON UNPAID ORDER
        old = conn.execute(
